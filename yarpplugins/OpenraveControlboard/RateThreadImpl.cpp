...
bool teo::OpenraveControlboard::threadInit() {
    printf("[OpenraveControlboard] success: threadInit()\n");
...
void teo::OpenraveControlboard::run() {
    for(unsigned int motor=0;motor<axes;motor++){
        ...
        if((jointStatus[motor]==1)||(jointStatus[motor]==2)||(jointStatus[motor]==3)) {  // if set to move...
            if ((getEncExposed(motor) > maxLimit[motor])  && (velRaw[motor]>0)) {  // SW max JL
                ...
                fprintf(stderr,"[OpenraveControlboard] warning: Moving joint q%d at configured max joint limit, stopping.\n",motor+1);
            } else if ((getEncExposed(motor) < minLimit[motor]) && (velRaw[motor]<0)) {  // SW min JL
                stop(motor);  // puts jointStatus[motor]=0;
                fprintf(stderr,"[OpenraveControlboard] warning: Moving joint q%d at configured min joint limit, stopping.\n",motor+1);
                ...
                if ( (velRaw[motor] > 0) &&  // moving positive...
                    ...
                    stop(motor);  // puts jointStatus[motor]=0;
                    printf("[OpenraveControlboard] Joint q%d reached target.\n",motor+1);
